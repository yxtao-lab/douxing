...
| 问题 | 方案 |
|------|------|
| 手帐只展示部分 `attractions`，无交通/住宿 | Payload 改用 `buildRouteDayFlow`；每天最多 8 节点 / 5 天 |
| 日记每天只 1 条、固定画布裁切 | 模板列全节点；`measurePosterHeight` 两阶段测高；移除日记提前 `break` |
...
```text
buildPosterPayload(route)
  → RoutePosterSheet：query Canvas(1334) → measurePosterHeight(ctx)
  → 设置 canvas 实测高度 → 再 query → loadPosterImages → renderPoster
```

### 审核要点

- [ ] 与详情「行程路径」节点顺序、类型一致  
- [ ] 2 天及以上路线在 `diary-page` 下可见多张日卡片  

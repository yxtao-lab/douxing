...
| 项 | 说明 |
|----|------|
| 本仓库 | 已含 `packages/ml-training/` 与 `pnpm ml:*` 脚本 |
...
| 训练方式 | **SFT 监督微调** |
| 微调方法 | **LoRA**（不要全参 SFT） |
| 训练数据集 | OSS 路径 → `train.jsonl` |
| 验证数据集 | OSS 路径 → `val.jsonl`（可选） |
...
| `seq_length` / `max_length` | **4096** | 与 Prompt 长度匹配 |
| `per_device_train_batch_size` | **2** | OOM 则改为 1 |
...
在 `packages/server/src/config/llm.ts` 中扩展 provider（若尚未合并）：

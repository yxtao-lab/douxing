...
> **推荐路径（现网）**：**百炼 DashScope 模型** → `DOUXING_LLM_*` → 兜行 API `auto` 链优先调用。  
> **推荐路径（后期 I2）**：项目内造数据 → PAI LoRA 训练 → 百炼注册自训 checkpoint（**Step 35 延后**）
> **文档版本**：1.3  
> **更新日期**：2026-07-06  
> **代码状态**：**I3 百炼接入已落地**（`douxing` provider · 现网 `DOUXING_LLM_*`）；**I2 PAI LoRA（Step 35）延后**（PAI 平台问题，后期再训）

...
| 环境 | API 地址 | 前端 env 文件 |
|------|----------|---------------|
| 本地开发 | `http://127.0.0.1:3000/api` | `.env.development` |
...
| 层级 | 运行位置 | 配置来源 | 典型命令 |
|------|----------|----------|----------|
| 后端 API | 本地或服务器 | 根目录 `.env` | `pnpm dev:server` / `pnpm deploy:server` |
| 前端/H5/小程序 | **本地编译** | `.env.development` / `.staging` / `.production` | `pnpm dev:*` / `pnpm build:*` |
...
| `.env` | 本地/服务器后端运行时配置（数据库、密钥、端口） |
| `.env.development` | 本地 `pnpm dev:*` 前端 API 地址 |
...
同时启动：API、管理端 Web、H5、微信小程序 dev 编译。

**方式 B：按需单独启动**

```bash
pnpm dev:server        # 后端 http://127.0.0.1:3000
pnpm dev:web           # 管理端 http://localhost:5173
pnpm dev:mobile        # H5 http://localhost:5174
pnpm dev:mp-weixin     # 小程序 dev 编译
pnpm dev:ai-service    # Python AI 服务（可选）
pnpm dev:with-ai       # API + AI + 全端
```
...
| 微信小程序 | 微信开发者工具导入 `packages/mobile/dist/dev/mp-weixin` |
...
若已有腾讯云等证书，见 [deploy-production.md §8.3b](./deploy-production.md)。
...
|------|------|----------|----------|----------|
| 开发 | `pnpm dev:mp-weixin` | `.env.development` | `dist/dev/mp-weixin` | 开发者工具导入，不校验域名 |
...
| `pnpm dev` | 启动 API + Web + H5 + 小程序 dev |
| `pnpm dev:server` | 仅后端 |
...
| 场景 | 命令 |
|------|------|
| 生产后端发版 | `git pull && pnpm deploy:server --skip-docker --use-tsx` |
...
[ ] pnpm dev
[ ] 浏览器打开 localhost:5173 / 5174
[ ] 微信工具导入 dist/dev/mp-weixin
```
...
**改 `.env.development` 后小程序仍连旧地址？**  
重启 `pnpm dev:mp-weixin`，重新编译导入。

**真机 uploadFile 连 127.0.0.1？**  
当前是 dev 包。真机请打生产包 `pnpm build:mp-weixin`，或配置 `.env.development.local` 指向 HTTPS 测试 API。

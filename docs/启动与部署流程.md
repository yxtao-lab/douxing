...
**要点**：`VITE_*` 在**打包时**写入前端；`API_PUBLIC_BASE_URL`、JWT（含 `JWT_ACCESS_EXPIRES_IN` / `JWT_REFRESH_EXPIRES_IN`）、数据库等由**后端 `.env`** 在运行时读取。鉴权机制见 [双Token认证与无感刷新.md](./双Token认证与无感刷新.md)。
...
# 2. 后端配置
cp .env.example .env
# 编辑 .env：DATABASE_URL、JWT_SECRET、JWT_ACCESS_EXPIRES_IN、JWT_REFRESH_EXPIRES_IN 等
...
| 命令 | 说明 |
|------|------|
| `pnpm ml:generate-dataset` | DeepSeek + RAG 生成训练 JSONL |
| `pnpm ml:validate-dataset -- --split` | 校验并划分 train/val |
...
| `pnpm --filter @douxing/server agent:m2-accept` | M2 一键验收 |
| `pnpm --filter @douxing/server h5:m5-accept` | M5 行中智能一键验收 |

详见 [阿里云-兜行专属模型训练与部署.md](./阿里云-兜行专属模型训练与部署.md) · [AI路径规划路线图.md](./AI路径规划路线图.md) · [AI流程编排路线图.md](./AI流程编排路线图.md)。

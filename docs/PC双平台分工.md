...
| 默认体验账号 | `demo` / 注册用户 | `admin`（须 `admin` 角色） |
...
管理端 **不替代** 移动端或 PC 的任何 C 端流程。模块 B **商户接单**（远期）为第四入口（如 `/partner`），见 [发单接单平台.md](./发单接单平台.md)。
...
| 路线详情 | `/routes/:id` | ✅ | 流程图、地图动画、草稿编辑、发布/公开、互动 |
...
| 模块 | 说明 | 文档 |
|------|------|------|
| **DT5 旅行运营大屏** | 全屏 `/screen/travel`；全国分布 + 用户路径漏斗 | [数据中台 §9](./数据中台.md#9-dt5-旅行运营大屏规划) |
| **F2 3D 资产审核** | glTF/GLB 入库、绑定景点；用户在 pc 播放 | [数字孪生与三维建模.md](./数字孪生与三维建模.md) |
| **模块 B 监管** | 商户入驻审核、需求/订单监管、分佣规则 | [发单接单平台.md §6.3](./发单接单平台.md#63-平台管理端现有-web) |
| **S1 RBAC 闭环** | `perms` 接口校验、动态菜单、数据权限 | [系统管理.md §2.4](./系统管理.md#24-鉴权现状与缺口) |
...
| 功能 | 用户端 PC | 管理端 Web |
|------|-----------|------------|
| **路线** | 卡片流 + Tab（我的/广场/收藏）；可 **编辑、发布、互动** | `DouxingAdminTable` 全站分页；**只读审计** |
| **订单** | **仅我的**；卡片列表；待支付可 **发起支付** | **全平台**表格；纯监管，无支付按钮 |
| **打卡列表** | **仅本人**记录 | **全用户**列表，运营排查 |
| **打卡地图** | 个人足迹；时间/城市筛选；全国 `fitBounds` | 全平台足迹；侧栏筛选；运营看分布 |
| **登录** | 普通用户；未登录可浏览首页部分能力 | 须 `admin` 角色；进工作台 |

...
**最后更新：** 2026-07-13（**M7-α 商户工作台** · Web `/partner` · 双 Token · C7-W · H10 · S1/S2 · DT5）
...
| **模块 B 发单** | 「我要发单 / 服务大厅 / 入驻」C 端入口 | [发单接单平台.md §6.1](./发单接单平台.md#61-c-端--pc) |
...
```text
用户端（pc + mobile）：我能做什么、我的数据、创作与分享、支付与会员
管理端（web）：全站数据怎么看、内容怎么审、规则怎么配、系统怎么运维
```

| 判断问题 | 放用户端 | 放管理端 |
|----------|----------|----------|
| 数据范围 | 「我的」或「广场公开」 | 「全平台 / 全用户」 |
| 操作性质 | 创作、消费、社交、支付 | 审核、启停、配置、监控 |
| 角色 | 注册用户 | `admin` + 未来 `perms` 细粒度权限 |
| 沉浸体验 / 3D | pc 用户浏览（F 线） | web 资产审核 + DT5 大屏 |
| 商户组织 | C 端发单/浏览标品 | 入驻审核、订单监管（M 线） |

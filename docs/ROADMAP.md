...
| **P0** | **A-CLOSE-01** | **A↔B 弱耦合增强：「按此路线发定制需求」** | 意向 → 落地履约闭环 | 模块 A 与 B 完全独立，规划结果无法一键带入发单约束 | 移动端/PC 路线详情 + `/api/marketplace/demands` | 已保存/公开路线出现「按此路线定制」入口；一键带出城市、天数、人数、预算、关键 POI；不强制串联 | [发单接单平台.md](./发单接单平台.md) · ROADMAP §M |

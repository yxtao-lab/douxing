...
| **P1** | **A-COGNITION-01** | **统一用户旅行画像（可解释的旅行人格）** | 用户认知资产 | 偏好、宠物记忆、历史路线、打卡、相册主题分散，未跨行程统一 | 后端画像服务 + 规划前/行中注入 | 有聚合画像表/服务；规划前可注入「旅行人格」上下文；画像字段可解释（节奏、预算弹性、同伴结构、忌讳等） | [AI规划与Agent演进.md](./AI规划与Agent演进.md) · C7-c 记忆 |

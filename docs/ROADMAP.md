...
| **当前焦点** | **H2-a+** 手帐轻量自定义 · **A2+++ Phase 2** OSS | H2-a + D5-a + **D5 补全（v0.8.7）** + v0.8.6 体验扫尾已完成；排期见 [下一步工作.md](./下一步工作.md) |
...
> **设计约定**：App 内仍 **旅行探索感**（蓝绿 token）；**分享图**另用手帐纸感 + 米黄主卡（见 [H2-a 模板](#h2-a-路线手帐海报模板体系)）。海报二维码 **三层兜底**：H5 链接 QR → 服务端小程序码 PNG → 微信 URL Link + 客户端 QR（v0.8.7，见 [§ D5 补全](./开发记录-重难点与亮点.md#d5-补全--海报二维码与行程全量展示)）。  

...
| **当前焦点** | **宣传类目 H1-a/b 已交付** · 继续 **H2-a** 路线海报 | 空状态组件已全站复用；下一步 H2-a + D5-a |
| **下一步建议** | **H1-a** 首屏与品牌 → **H2-a** 路线海报 + **D5-a** 分享链路 → **H1-b/c** 全站统一 | 宣传 Sprint 后再排 **H8** 行中重规划 · **E2** 真支付 · [H9+ 远期增强](#h9-远期增强phase-4-之后) |
...
| 待定 | **H1-c** | 宣传 Phase 4 — 全站视觉统一 | 规划/路线/我的/登录皮肤；品牌化 Loading 与成就动效 |
...
| §13 | 前沿技术 | — | Web3、联邦学习等 |
| 移动端 UX | iconfont TabBar、四 Tab | **宣传类目 H1-a～c**（品牌 token、首页 Hero、空状态、全站皮肤）；[§5 宣传类目](#宣传类目h1--h2--d5-最小版分-phase-实施) | 自定义主题、消息红点 |
...
| 步 | 状态 | 计划完成 | 名称 | 设计文档 | 交付内容 | 验收标准 |
|----|------|----------|------|----------|----------|----------|
| H1 | [~] | — | 界面包装（总览） | 产品体验 | **H1-a～c** 分 Phase；见 [宣传类目](#宣传类目h1--h2--d5-最小版分-phase-实施) | 主流程页面风格一致；切换 zh-CN/en-US 无布局破版 |
...
| **H1-c** | [ ] | 待定 | **Phase 4 — 全站视觉统一** | H1-a、H1-b | 规划页、路线列表、我的、登录页统一皮肤；`AiPlanBlockingOverlay`、成就解锁等关键动效品牌化 | 主流程视觉一致；切换 en-US 无布局破版 |
...
| ID | 名称 | 类别 | 关联 §5 | 依赖 | 状态 |
|----|------|------|---------|------|------|
| H1 | 界面包装 | 体验 / 视觉 · **宣传** | [H1-a～c](#宣传类目h1--h2--d5-最小版分-phase-实施) | G9 i18n | [~] |

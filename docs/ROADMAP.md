...
**新增专题**：[系统管理.md](./系统管理.md) · [发单接单平台.md](./发单接单平台.md) · [数字孪生与三维建模.md](./数字孪生与三维建模.md)
...
3. [当前实现概况](#3-当前实现概况)
4. [模块对照总表](#4-模块对照总表)
5. [分步实施路线图](#5-分步实施路线图)（含 [阶段 P：PC 用户端](#阶段-pc-用户端c-端桌面网页2026-06-08-录入) · [阶段 S：系统管理](#阶段-s系统管理2026-06-09-录入) · [阶段 M：发单接单](#阶段-m发单接单平台2026-06-09-录入) · [阶段 F：数字孪生/3D](#阶段-farvr-数字孪生与三维建模可后置)）
...
| **当前焦点** | **J1 旅程相册** · 开发环境联调 · **H9-3 扫尾** · **PC P5 部署**（远期） | **G4/DT1 已完成**（2026-06-09）；P0～P4、双端品牌已落地；见 [下一步工作.md](./下一步工作.md) |
| **下一步建议** | 见 **[下一步工作.md](./下一步工作.md)** | 联调 → H9-3 → **J1** → DT3/DT2 按需 → 行中 H8/H7 |
...
| **PC 用户端** | 2026-06-10 | **`packages/pc`**：**P0～P4 + P3+ 打卡地图** — 脚手架 · 规划 · 路线详情 · 个人中心子页 · **Leaflet 打卡地图** · 分享手帐 PNG · `pnpm dev:pc` / `:5176` |
| 管理端 | 2026-06-10 | 路线、订单、打卡列表 + **打卡地图**（`/checkins/map`，Leaflet 全平台足迹）；**兜行管理后台** 品牌（Logo1 青蓝+橙 · Ant Design 主题 `#008cba`） |
...
| **§8 + Web** | **系统管理（S 线）** | **S0 进行中**：`sys_menu`/`sys_dept`/用户/角色/菜单 CRUD + Web 页；鉴权仍 `requireAdmin` | **S1** `role_menu` + `requirePerm` + 动态菜单；见 [系统管理.md](./系统管理.md) |
...
| 步 | 状态 | 计划完成 | 名称 | 依赖 | 交付内容 | 验收标准 |
|----|------|----------|------|------|----------|----------|
| **S0** | [~] | — | 管理骨架 | W1、G9 | 表迁移 + `/api/system/*` CRUD + Web 系统/监控/日志页 + `requireAdmin` | 管理员可维护用户/角色/菜单/部门等 |
...
| 平台治理 | 贯穿 | **S（系统管理）** | S0 骨架进行中；S1 RBAC 待做 |
...
| **3D / 旅行孪生** | **F0**（底座 + PC 路线 3D）→ **F1** 全景 → **F2** 模型 → **F3** 讲解 → **F4** 足迹孪生 | 见 [数字孪生与三维建模.md](./数字孪生与三维建模.md)；J1/P5 后启动 |
...
| **系统管理（Web 后台）** | **S0 收尾** → **S1 RBAC** → S2 动态菜单 | 见 [系统管理.md](./系统管理.md) |
...
| **S** | 系统管理（平台 RBAC） | 平台 / 安全 | [S0～S4](#阶段-s系统管理2026-06-09-录入) | W1、G9 | [~]（**S0 骨架进行中**，S1 鉴权待做） |
...
规划对话（C1）          搭子社交（D2/D3 · 非商业）
        │                        │
        └─ H3 角色 ── H9 ── H8/H7 ── B2 打卡 ── H2 手帐 ── J 相册
                              │
                    H10 视频/热评 ── A2 景点
                              │

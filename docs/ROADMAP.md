...
| **当前焦点** | 阶段 A：补齐 MVP 缺口 | 用户资料、景点库、路线能力、订单状态机 |
| **下一步建议** | A1 → A2 → A4 | 资料完善 + 景点库 + 订单可靠 |
| **完成度（里程碑）** | M0：**6/6** 已完成 · A～G：**0/38** 未开始 | 见 [时间轴](#2-项目开发进度时间轴) |
...
| 用户认证 | 2026-05-19 | 注册、登录、JWT、`/auth/me`、RBAC（admin / user） |
...
| 移动端 | 2026-05-20 | Tab：首页 / 规划 / 路线 / 我的；iconfont 底栏；登录注册 |
...
|----|------|----------|------|----------|----------|----------|
| A1 | [ ] | 2026-05-24 | 用户资料与偏好 | §7.2.1、§12.2.1 | `PUT /users/me`、头像、兴趣标签；移动端编辑页 | 修改资料持久化 |
...
|----------------|-------------|------------|----------|
| Sprint 1–2 | MVP | **M0（已完成）** | 2026-05-19 ～ 2026-05-20 |
...
| 迭代 | 计划周期 | 步骤 | 目标 |
|------|----------|------|------|
| 迭代 0 | 2026-05-19 ～ 05-20 | M0-1 ～ M0-6 | **已完成** — MVP 可演示、多端可跑 |
...
2. **每发布版本**：在 §2.2 追加版本行（如 v0.3.2）。  
3. **计划变更**：只改 §2.3 与 §5「计划完成」列，保留历史于 Git 提交记录。  
4. **README**：MVP / API 有重大变更时同步 [README.md](../README.md)。

---

*文档版本 2.0 · 最后更新：2026-05-20*

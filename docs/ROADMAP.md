...
| **当前焦点** | **H2-a+** 手帐轻量自定义 · **A2+++ Phase 1** 景点封面自动补全 | **H2-a + D5-a 已验收**（2026-06-01）；**A2+++ Phase 0** 运营封面上传已落地 |
| **下一步建议** | **H2-a+**（副标题/配色/POI 条数/贴纸）→ 可选并行 **I2** · **A2+++ Phase 1**（高德 POI 图 + OSS） | 全功能拖拽编辑器归 **H2-c 远期**；宣传 Sprint 后再排 **H8** · **E2** |
...
| v0.8.4 | 2026-06-01 | A2+++ 景点封面 Phase 0 | `attractions` 封面字段 · 管理端上传 · 路线 POI/手帐/分享页展示 · Image Enricher 骨架；见 [§ A2+++](./开发记录-重难点与亮点.md#a2-景点封面-phase-0) |
...
| 2026-06-01 | A2+++ 景点封面 Phase 0 | [§ A2+++](./开发记录-重难点与亮点.md#a2-景点封面-phase-0) |
...
| 路线管理 | 2026-06-01 | 生成、列表、详情、发布；解锁可选；**详情页地图预览**（服务端 polyline）+ **行程流程图**；规划对话区仅消息与候选卡片；草稿 **hero 编辑弹窗**；**手帐分享海报** + **H5 只读分享页** |
| 景点封面 | 2026-06-01 | `attractions` 封面字段；管理端上传；路线 POI/手帐/分享页展示（Phase 0 运营上传，不接入爬虫） |
...
| A2+++ | [x] | 2026-06-01 | 景点封面 Phase 0 | §3.5 | `cover_image_url` 等字段 · 管理端上传 · 路线/手帐/分享页消费 · Enricher 骨架 | 运营上传封面后，详情流程图/手帐/分享页可见缩略图 |
| A2+++ P1 | [ ] | 待定 | 景点封面自动补全 | §3.5 | 高德 POI 图 · Wikimedia fallback · OSS · UGC 聚合 | 无运营图时自动补封面；带来源与授权字段 |
...
| H2 | [~] | 2026-06-01 | 分享手帐海报（总览） | §4、D5 | **H2-a + D5-a 已完成**；**H2-a+ / H2-b / H2-c** 待续；见 [宣传类目](#宣传类目h1--h2--d5-最小版分-phase-实施) | 多模板手帐 PNG + 二维码；H2-c 拖拽编辑为远期 |
...
> **设计约定**：App 内仍 **旅行探索感**（蓝绿 token）；**分享图**另用手帐纸感 + 米黄主卡（见 [H2-a 模板](#h2-a-路线手帐海报模板体系)）。海报二维码 **短期 H5 只读链接、中期小程序码 API**（不阻塞 H2-a MVP）。  
...
| **D5-a** | [x] | 2026-06-01 | **分享链路最小版** | H2-a | 海报右下角 **H5 只读链接**（短期，配置 `VITE_H5_BASE_URL`）；`GET /api/share/routes/:id` 只读详情页（未登录可浏览已公开路线） | 扫码或打开 H5 链接可只读浏览路线摘要；与 H2-a 海报二维码打通 |
...
| **H2-a+** | [ ] | H2-a 后 1 周内 | **手帐海报轻量自定义** | H2-a | 在已选模板上可调：**副标题**、**主题色预设**（2～4 套）、**每天展示 POI 数**（2/3/4）、是否显示时间；角标贴纸（旅行/美食/亲子等 PNG） | 不改版式、不拖拽；重新生成 PNG；选项 i18n |
| **H2-b** | [ ] | 待定 | **Phase 5 — 打卡/成就炫耀海报** | H2-a、B3、B4 | 复用 `poster/` 管线；打卡地图缩略 + 徽章/成就模板；成就页、打卡地图页入口 | 用户可生成打卡/成就炫耀图并分享 |
| **H2-c** | [ ] | 远期 | **手帐画布编辑器（真·自定义）** | H2-a+ | 拖拽贴纸/照片框/文字位；保存「我的模板」；可选服务端存储 | **不纳入 H2-a MVP**；单独立项，避免与 6/10 宣传验收捆绑 |

**推荐实施顺序（宣传类目）**：`H1-a → H1-b → H1-c`（已完成）→ **`H2-a + D5-a`**（已完成）→ `H2-a+`（下一步）→ `H2-b`；`H2-c` 视资源后置。

**当前 Sprint（2026-06-01 起）**：**H2-a+** 轻量自定义 · 可选并行 **A2+++ Phase 1**（高德 POI 图 + OSS）· **I2**。
...
| `diary-page` | 螺旋本日记 | 按 Day 分段 + 拍立得框 + 胶带装饰 | 每天 1～2 POI + 短文案；图用 `poiType`/占位插画 | **MVP 必做** |
...
| 轻量选项 | **H2-a+** | 副标题、配色预设、POI 条数、贴纸，无拖拽 |
...
| 优先级 | 步骤 | 理由 |
|--------|------|------|
| **P0** | **H2-a+** | 副标题/配色/POI 条数/贴纸；H2-a 已验收 |
| P0.5 | **A2+++ Phase 1** | 高德 POI 图 + OSS 自动补封面；与 H2-a+ 可并行 |
| P0 | H8、H7 | H9 四 Phase 已交付；宣传 Sprint 后推进行中智能 |
| P0.5 | [I2、I3](#阶段-i专属模型2026-06-01-录入) | 专属模型 PAI 微调与百炼接入；与 H2-a 可并行（不同人力） |
| P0.5 | [H9+-1、H9+-2](#h9-远期增强phase-4-之后) | Playbook 库扩充 + Web CRUD；见 H9+ 表 |
| P1 | H2-b | 打卡/成就炫耀海报；复用 `poster/` |
...
| 顺序 | 任务 | 预估 | 说明 |
|------|------|------|------|
| 1 | `poster/types` + `build-poster-payload.ts` | 0.5d | 从 `TravelRouteInfo` / `routeDetail.days` 抽统一 Payload；截断过长 `description` |
...
| 5 | D5-a 二维码绘制（H5 URL） | 0.5～1d | 依赖 `VITE_H5_BASE_URL`；无 H5 时仅小程序卡片分享 |
| 6 | `templates/diary-page.ts` | 1～1.5d | 第二套 MVP；POI 占位贴纸 |
| — | `diary-page` 可并行于 2 之后 | — | 人力紧则先上线 `timeline-columns` 再补日记页 |

**本阶段不做**：H2-c 拖拽编辑、`path-scrapbook` 曲线排版、每 POI 定制水彩图、后端小程序码（可 D5-a 二期）。
...
| — | A2+++ P1 | 高德 POI 图 + OSS 自动补全 | 待定 |
| 6+ | H10-a～c | 景点说明+热评 → UGC 短视频 → 详情/手帐跳转 | H2-a、D5-a 之后 |

**当前 Sprint 范围**：**H2-a+** 轻量自定义 · 可选并行 **A2+++ Phase 1** · **I2**；Phase 1/2/3/4（H1 + H2-a + D5-a）已完成。  

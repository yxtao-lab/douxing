...
| **当前焦点** | **H2-a + D5-a** 路线手帐海报与分享（下一步） | **H1-c 已验收**（2026-06-01）；H2-a 定为 **多模板手帐风** + 模板选择（见 [§5 H2-a 模板](#h2-a-路线手帐海报模板体系)） |
| **下一步建议** | **H2-a MVP**（2 模板 + D5-a）→ **H2-a+** 轻量选项 → 可选并行 **I2** | 全功能拖拽编辑器归 **H2-c 远期**；宣传 Sprint 后再排 **H8** · **E2** |
...
| 2026-06-10 | **H2-a + D5-a** | 宣传 Phase 3 — 路线手帐海报与分享链路 | 模板选择 + Canvas（默认「分列行程」+「日记页」）；`onShareAppMessage`；海报 H5 二维码（D5-a） |
...
| §13 | 前沿技术 | — | Web3、联邦学习等 |
| 移动端 UX | iconfont TabBar、四 Tab | **H1-a/b/c 已完成**；全站 `--dx-*` token + 双主题；见 [§5 宣传类目](#宣传类目h1--h2--d5-最小版分-phase-实施) | **H2-a** 路线手帐海报（多模板）；Web 管理端视觉未纳入 |
| 分享与传播 | 广场公开路线 | **H2-a～c + D5-a**；远期 **H10** UGC 视频+热评增强可信度 | D5 完整 H5 只读页 + 搭子链分享 |
...
| H2 | [ ] | — | 分享手帐海报（总览） | §4、D5 | **H2-a～c**、**H2-a+**、**D5-a**；见 [宣传类目](#宣传类目h1--h2--d5-最小版分-phase-实施) | 多模板手帐 PNG + 二维码；H2-c 拖拽编辑为远期 |
...
| **H2-a** | [ ] | 2026-06-10 | **Phase 3 — 路线手帐海报（模板选择）** | H1-a、H1-c、A3 | 路线详情「生成分享图」；`packages/mobile/src/poster/` 统一数据层 + 模板注册表；**MVP ≥2 套模板**（见下表）；Canvas 导出 PNG；保存相册；`onShareAppMessage` | 可选模板 → 预览 → 保存/转发；含路线名、天数、亮点 POI、品牌底栏、二维码；zh-CN/en-US 不破版；好友进路线详情 |
...
**推荐实施顺序（宣传类目）**：`H1-a → H1-b → H1-c`（已完成）→ **`H2-a + D5-a`** → `H2-a+`（可选）→ `H2-b`；`H2-c` 视资源后置。

**H2-a 当前 Sprint（约 5～7 天）**：模板选择 + 2 套 Canvas 手帐 + 小程序转发 + D5-a 二维码（H1-a 已完成，不再重复排期）。
...
| 迭代 10e | 2026-06 上旬 | **宣传类目 Sprint** | **H2-a + D5-a** — 手帐多模板海报 · 小程序分享 · H5 二维码（H1-c 已完成） |
| 迭代 10f | 2026-06-01 | **I1** | **已完成** — ML 训练数据生成与校验管线 |
| 迭代 10g | 待定 | **I2 + I3** | PAI LoRA 微调 · 百炼推理接入 · A/B 对比 |
| 迭代 11a | 2026-06 上旬 | H2-a + D5-a | 路线手帐海报（2 模板）· 分享链路 |
...
| 优先级 | 步骤 | 理由 |
|--------|------|------|
| **P0** | **[H2-a、D5-a](#宣传类目h1--h2--d5-最小版分-phase-实施)** | 模板选择 + `timeline-columns`/`diary-page` + 保存转发；目标 2026-06-10 可对外晒图 |
| P0.5 | H2-a+ | 副标题/配色/POI 条数/贴纸；不阻塞 H2-a 验收 |
...
| 3 | H2-a + D5-a | 手帐多模板（≥2）、模板选择、小程序转发、H5 二维码 | 2026-06-10 |
| 3+ | H2-a+ | 副标题/配色/POI 条数/贴纸（轻量自定义） | H2-a 后 1 周 |
...
**当前 Sprint 范围**：Phase 3（`H2-a + D5-a`），约 5～7 天；Phase 1/2/4（H1）已完成。  

...
**文档版本**：4.7  
**最后更新**：2026-07-17（**M1～M6 主链已达成** · Step 39 **Web 审核 UI ✅** · Step 35 PAI 延后 · 现网百炼 · **精准化与 AI 流程优化联动 21 点录入**）
...
| **I** | 专属模型 | 🔄 I1 ✅ · I3 基础设施 ✅ | Step 35～37 |
| **H10** | 路线可信度 | 🔄 a/b/c ✅ · d 待做 | Step 38～41 |
...
### 10.1 精准化与 AI 流程优化联动（2026-07-17 录入）

> **来源**：[精准化与人群定制路线图.md §13](./精准化与人群定制路线图.md#13-ai-流程优化联动2026-07-17-录入) · 标签体系（P-TAG-01 / P-ONBOARD-01 / P-INPUT-01 / P-MEMORY-01）引入后的 21 个 AI 流程优化点。

**嵌入本路线图的优化点**：

| 优化点 | 嵌入 Step | 说明 |
|--------|-----------|------|
| 1. 标签直接构建意图跳过 LLM | P-INPUT-01 落地时 | `TravelIntentSnapshot.source='manual'` |
| 2. 标签+文本混合强约束合并 | P-INPUT-01 落地时 | `source='hybrid'` |
| 3. C2 新增 sceneTags 解析 | P-TAG-01 落地时 | C2 解析扩展 |
| 4. RAG topK 重排引入场景匹配分 | P-TAG-01 落地时 | C3 topK 重排 |
| 5. 避重过滤前置到 RAG | P-MEMORY-01 落地时 | RAG `WHERE NOT IN` |
| 6. RAG 参数按人群动态调整 | P-ONBOARD-01 落地时 | topK/variants 动态 |
| 7. Prompt 注入画像/场景/已去过 | P-ONBOARD-01 落地时 | prompt 模板扩展 |
| 8. 按场景标签分支 Prompt 模板库 | P-TAG-01 落地时 | 模板 CRUD |
| 9. 新增 tweak_tags 意图路由 | P-INPUT-01 落地时 | C7-b 路由扩展 |
| 10. Agent 主动读取画像建议 | P-ONBOARD-01 落地时 | C7-c 画像注入 |
| 11. 段间交通选型参考场景 | P-TAG-01 落地时 | H9-4d 增强 |
| 12. 住宿选址参考同伴结构 | P-ONBOARD-01 落地时 | H9 酒店选址 |
| 13. 工作流模板按场景分支 | P-TAG-01 落地时 | W3 模板增强 |
| 14. 候选方案按场景差异化 | P-TAG-01 落地时 | C4 多方案 |
| 15. 重规划替补参考场景 | P-TAG-01 落地时 | H7 替补 |
| 16. 行中救场结合画像 | P-ONBOARD-01 落地时 | A-COGNITION-03 |
| 17. Golden Case 按人群分桶 | A-COGNITION-02 落地时 | 反馈环 |
| 18. 训练数据分层采样 | I1/I3 落地时 | 训练数据 |
| 19. 画像刷新纳入场景偏好 | P-ONBOARD-01 落地时 | 画像沉淀 |
| 20. 记忆召回按场景过滤 | P-TAG-01 落地时 | C7-c 记忆 |
| 21. 标签约束贯穿降级链 | 全线落地时 | 降级链 |

**落地策略**：

- **P0（必须，与标签待办同步）**：优化点 1、5、7、21
- **P1（标签待办后第一迭代）**：优化点 2、3、4、8、11、15
- **P2/P3（后续迭代，按需）**：其余优化点

**与 M6 关系**：21 个优化点不阻塞 M6 主链声明（Step 35 延后已不影响）；P0 项随精准化待办落地，P1/P2/P3 作为 M6 后迭代。
...
| 2026-07-17 | 4.8 | **精准化与 AI 流程优化联动**：新增 §10.1，标注 21 个优化点嵌入位置（P0 随标签待办同步，P1/P2/P3 后续迭代）；关联 [精准化与人群定制路线图.md §13](./精准化与人群定制路线图.md#13-ai-流程优化联动2026-07-17-录入) |

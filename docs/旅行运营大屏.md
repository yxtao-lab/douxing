...
**文档版本**：1.2  
**更新日期**：2026-07-01  
**状态**：一期已落地（2D 地图 + 漏斗 + KPI + 热力图层 + 时间口径角标）；全球 3D 地球 + 层级下钻 + 动向飞线为 **二期规划**
...
1. **页头**：标题 + 实时时钟 + **时间口径角标**（`geoScopeBadge`：近 N 天 / 全量回退）  
...
| `TravelGeoMap` | ECharts Map + Lines + EffectScatter + **Heatmap** | 省着色、城市散点、城际飞线、**网格热力点** |
...
| heatmap | heatmap | 打卡网格热力（`heatPoints`，0.1° 聚合 Top 200） |
...
- 大屏 `TravelGeoMap` 以 ECharts **heatmap** 图层消费（2026-07-01 已接入）

**响应时间口径**（2026-07-01）：

| 字段 | 说明 |
|------|------|
| `scopeDays` | 请求参数 `days`（1～90） |
| `effectiveScope` | `window` = 时间窗内有省/市数据；`allTime` = 时间窗为空时回退全量已通过打卡 |

页头 `geoScopeBadge` 随 `effectiveScope` 展示「近 N 天」或「全量数据（时间窗无打卡）」。
...
- `AnalyticsOverview`
- `AnalyticsGeoDistribution` / `AnalyticsProvinceGeoStat` / `AnalyticsCityGeoStat` / `AnalyticsHeatPoint` / `AnalyticsGeoEffectiveScope`
- `AnalyticsGeoFlow`
- `AnalyticsFunnelStep`
...
| 地图 / 漏斗 / 城际流动 | 近 30 天 | 页头 `geoScopeBadge` 展示实际口径 |
| 地理分布（回退） | 全量 | `effectiveScope=allTime` 时角标提示 |
...
| 全国旅行分布 | ✅ | ECharts 省级着色 + 城市点 + 飞线 + **热力图层** |
| 打卡热力 | ✅ | ECharts heatmap 消费 `heatPoints` |
| 用户路径漏斗 | ✅ | ECharts 漏斗（非桑基） |
| 城际 OD | ✅ 部分 | 列表 + 飞线；阈值偏高 |
| 3D 底图 | ❌ | Cesium / 3D Tiles 未做 |
| 时间范围切换 | ❌ | 固定 30 天（API 已支持 `days` 参数） |
| 实时推送 | ❌ | 仅 60s 轮询 |
| 数据范围标注 | ✅ | `scopeDays` + `effectiveScope` + 页头角标 |
...
| 优先级 | 项 | 方案 |
|--------|-----|------|
| P0 | ~~**统一时间口径展示**~~ | ✅ API `scopeDays` / `effectiveScope` + 页头 `geoScopeBadge` |
| P0 | ~~**打通热力图层**~~ | ✅ ECharts heatmap 消费 `heatPoints` |
...
- [ ] 漏斗 6 步与埋点事件一致
- [x] `pnpm --filter @douxing/server dt5:analytics-cases` 通过
- [ ] 无权限用户被路由守卫拦截
- [ ] 回退全量时页头 `geoScopeBadge` 显示正确口径
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-07-01 | 1.2 | 地理统计时间口径（`scopeDays` / `effectiveScope`）；地图热力图层；页头角标 |

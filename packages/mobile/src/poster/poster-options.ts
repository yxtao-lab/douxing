...
export const POSTER_OPTIONS_STORAGE_KEY = 'douxing_poster_render_options';
...
export function loadStoredPosterRenderOptions(): PosterRenderOptions {
  try {
    const raw = uni.getStorageSync(POSTER_OPTIONS_STORAGE_KEY) as string;
...
export function savePosterRenderOptions(options: PosterRenderOptions): void {
  try {
    uni.setStorageSync(POSTER_OPTIONS_STORAGE_KEY, JSON.stringify(options));
  } catch {
    // 存储失败可忽略
  }
}

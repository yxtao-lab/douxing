...
import type {
  ...
  AnalyticsGeoFlow,
  AnalyticsHeatPoint,
  AnalyticsProvinceGeoStat,
} from '@douxing/shared';
import type { EChartsOption } from 'echarts';
import * as echarts from 'echarts/core';
import { MapChart, LinesChart, EffectScatterChart, HeatmapChart } from 'echarts/charts';
import {
  GeoComponent,
  TooltipComponent,
  VisualMapComponent,
...
function buildOption(): EChartsOption {
  ...
  const heatData = props.heatPoints
    .filter((point) => Number.isFinite(point.latitude) && Number.isFinite(point.longitude))
    .slice(0, 120)
    .map((point) => [point.longitude, point.latitude, point.weight] as [number, number, number]);
...
watch(
  () => [props.provinces, props.cities, props.flows, props.heatPoints, props.emptyLabel] as const,
  () => {
    renderChart();
  },
  { deep: true },

#!/usr/bin/env bash
# 兜行 · Ubuntu 22.04 LTS 服务器环境初始化
# 用法: sudo bash scripts/setup-ubuntu22.sh
# 适用：腾讯云轻量应用服务器重装 Ubuntu 22.04 后首次配置
set -euo pipefail

if [[ "${EUID:-0}" -ne 0 ]]; then
  echo "请使用 root 或 sudo 执行: sudo bash scripts/setup-ubuntu22.sh"
  exit 1
fi

echo "========================================"
echo "  兜行 · Ubuntu 22.04 环境初始化"
echo "========================================"
...
echo "[2/8] 安装基础工具..."
apt-get install -y git curl wget vim ca-certificates gnupg lsb-release ufw
